...
export function TaskItem({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'task-item group relative flex items-start gap-4 rounded-2xl p-4 sm:p-4',
    ...
    >
      ...
      {task.priority !== 'none' && (
        <div className={cn(
          ...
          task.priority === 'high' && 'bg-gradient-to-b from-rose-500 to-rose-400',
          task.priority === 'medium' && 'bg-gradient-to-b from-amber-500 to-amber-400',
          task.priority === 'low' && 'bg-gradient-to-b from-sky-500 to-sky-400',
        )} />
      )}
      ...
      >
        ...
        >
          <Checkbox
            ...
            className={cn(
              'task-checkbox h-6 w-6 sm:h-5 sm:w-5 rounded-full',
              'transition-all duration-300',
              'border-2',
              ...
              task.completed && 'border-primary bg-gradient-to-br from-primary to-accent'
            )}
          />
        </motion.div>
      </div>
      ...
      <div className="flex-1 min-w-0 space-y-2">
        ...
        <div className="flex items-start gap-2">
          <span
            className={cn(
              'text-base sm:text-sm font-medium leading-snug',
              'transition-all duration-300',
        ...
        {/* Meta information */}
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm sm:text-xs">
          {/* Due date */}
          {task.date && (
            <span className={cn(
          ...
          {hasAttachments && (
            <span className="flex items-center gap-1.5 text-muted-foreground" title={`${task.attachments!.length} attachment(s)`}>
      ...
      {taskIsOverdue && (
        ...
        >
          <Badge 
            variant="destructive" 
            className="text-xs font-semibold shadow-lg shadow-destructive/20"
          >
